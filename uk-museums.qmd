...
```{r}
#| label: museums-basic-plot-1
#| message: false
library(ggplot2)
ggplot(
  data = plot_data,
  mapping = aes(x = year, y = museums, colour = deprivation)
) +
  geom_line()
```

speghetti plots
10 is a fairly large number of colours
the colours in the legend are in different order
...
let's go back to geom_line \index{gghighlight!gghighlight}

```{r}
#| label: museums-basic-plot-3
#| message: false
ggplot(
  data = plot_data,
  mapping = aes(x = year, y = museums, colour = deprivation)
...
```{r}
#| label: museums-basic-plot-4
#| message: false
base_plot <- ggplot(
  data = new_plot_data,
  mapping = aes(x = year, y = change, colour = deprivation)
) +
  geom_line() +
  facet_wrap(~deprivation, nrow = 2) +
  gghighlight::gghighlight(use_direct_label = FALSE)
...
```{r}
#| label: museums-colours
bg_col <- "#fafafa"
text_col <- "black"
```

```{r}
#| label: museums-style-plot-1
col_plot <- base_plot +
...
What would I want to change about this plot? colour probably not needed
