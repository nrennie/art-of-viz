...
Let's define some text for the title and subtitle. Here, the title is simple (it could perhaps be a little more descriptive or interesting). The subtitle gives a brief explanation of how to interpret the plot, and the main conclusion that we want the reader to take away. The `caption` variable identifies the source of the data for this visualisation, as well as the chart creator.
...
* As we've done in previous visualisations, we also set the background colors to `bg_col` and add some padding around the edges by setting the `plot.margin` argument.
...
Although we could simply increase the height of the plot to stop the year labels from becoming squashed, an alternative approach is to address the source of the issue and edit the legend. We have a few different options for dealing with the legend. Some options  might be:
...
* We could design a custom legend. This might be a good option as double gauge charts are not so common and readers might be less familiar with them. Adding additional information about how they work might prove helpful. We'll look at how to design and use a custom in @sec-house. 

* We could instead use colored text in the subtitle to indicate what the categories are. For this visualisation, highlighting the *Immunized* would be enough as there are only two categories. We'll look at ways to do this in R in @sec-turbines and @sec-cats. 
Here, there's an argument to be made that any form of legend is in fact unnecessary. The current choice of colors and title of the chart makes it clear enough that the purple color represents the positive response data (percentage who have been immunized). 

::: {#tip-technology-legends .callout-tip}

## Don't rely on legends

More specifically, don't rely on color alone for communicating information. Color blindness comes in many different form, and different types affects how people perceive colors in different ways. One way to check if your chart is accessible to people with color blindness, is to view it in grayscale (black and white) and consider if it can be understood without color. If it can't consider choosing different colors, or shapes and patterns to differentiate groups instead. \index{accessibility} \index{legend}

There are several packages in R that can be used to investigate the accessibility of color palettes. One of them is {colorblindr} [@colorblindr]. The {colorblindr} package allows you to create simulations of what your chart may look like to some one with color blindness. The package also includes colorblind friendly qualitative palettes with associated {ggplot2} functions. At the time of writing, {colorblindr} is not available on CRAN but can be installed from GitHub, as described in @sec-bees. \index{colorblindr!cvd\_grid}

```{r}
#| label: fig-technology-cvd
#| fig-cap: "A 2x2 grid of charts showing simulations of what our chart looks like with four different types color vision deficiency."
#| echo: -1
theme_set(theme_void(base_size = 4))
colorblindr::cvd_grid(styled_plot)
```

Note that the default color palette in {ggplot2} does not show any differentiation when viewed in grayscale, and so it's important to think about which colors you're using for any chart.

If you do have to use colors to differentiate groups, add direct labels to data where possible, and make sure that the order of the legend matches the order of the data [@GAcharts].
...
* Although a full, traditional legend is may be unnecessary here, the addition of some colored text may add clarification that the purple area is what represents that data. We'll look at using colored text instead of a legend in @sec-turbines and @sec-cats.
