...
An alternative to {remotes} is the {pak} package [@pak]. In fact, it is actually an alternative way to install R packages from CRAN, Bioconductor, GitHub, URLs, git repositories, local files and directories. To install a package from GitHub using {pak}, use the `pkg_install()` function and again pass in the `"user/repository"` string:
