...
#| fig-cap: "Poisson disk sampling and uniform sampling"
#| fig-subcap:
...
If we want to ... black points against a ayellow background.
...
Since this is already quite a visually complex chart, we'll avoid further distractions and use the same font for all text elements in the chart. This means we define only a single font variable, `body_font`. We'll also use a simple, easy-to-read font. The *Red Hat* fonts, again available through Google Fonts, are sans serif fonts, with the *Red Hat Text* version designed for increased legibility, especially at smaller font sizes.
...
We can then add this text using the `labs()` function, and setting the `title`, `subtitle`, and `caption`. The x- and y- axes are meaningless for this visualisation since the data is represented by the number of points rather than their locations. We remove the x- and y- axis labels by setting them both to `NULL` in @fig-bees-text-plot. \index{ggplot2!labs}
...
\index{ggplot2!coord\_fixed} \index{ggplot2!theme\_void}
