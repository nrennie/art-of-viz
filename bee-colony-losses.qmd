...
We want the area to be more populated and contain more points where the number of colonies is higher. This means the minimum radius between points should be smaller when the number of colonies is higher i.e. there is an inverse relationship between `r` and `colony_n`. The default grid in `poisson2d()` goes from 0 to 10 on both axes. The values in `colony_n` range from `r min(plot_data$colony_n)` to `r max(plot_data$colony_n)`, meaning that if we simply set the minimum radius to `1/colony_n`, we end up with points that are only `r format(1/max(plot_data$colony_n), scientific = FALSE)` apart. That's a bit too small! We can multiply by a constant factor to increase this distance. After some trial and error, we set `r` to `200000/colony_n`. \index{poissoned!poisson2d}
