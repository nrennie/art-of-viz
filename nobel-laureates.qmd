...
* Parameterise your plotting code to create a function, to make it easier to produce lots of similar graphics with a consistent layout.
...
An API (Application Programming Interface) is essentially a tool that lets different software programs communicate with each other [@harmon2024]. The Nobel Prize API endpoint [@nobel_data] can be accessed at {{< monolink "https://api.nobelprize.org/2.1/laureates" "api.nobelprize.org/2.1/laureates" >}}, with some instructions and guidance about how to use it available on the {{< monolink "https://www.nobelprize.org/" "nobelprize.org" >}} website at {{< monolink "https://www.nobelprize.org/organization/developer-zone-2/" "www.nobelprize.org/organization/developer-zone-2" >}}, which also links to the [Terms of Use](https://www.nobelprize.org/organization/terms-of-use-for-api-nobelprize-org-and-data-nobelprize-org/).
...
Constructing the base plot is reasonably straightforward - we've done the hard work in defining the x values, y values, the angles already. Since our plot is a visualization of text data, we'll use `geom_text()` to add the text. All we have to do is tell **`ggplot2`** what to put where! \index{ggplot2!geom\_text}
...
The primary color will be used to highlight the female laureates, as well as the title and caption text. The secondary color will be used to color the male laureate names. The primary color should be bolder or brighter than the secondary color - meaning a few pops of color on the chart will stand out and highlight the female laureates. \index{color}
...
#| fig-alt: "The names of Nobel Prize in Physics awardees arranged in an anti-clockwise direction, with female awardees highlighted in a darker color."
styled_plot +
...
#| fig-alt: "The names of Nobel Peace Prize awardees arranged in an anti-clockwise direction, with female awardees highlighted in pink."
