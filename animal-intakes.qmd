...
* **`scales`**: **`ggplot2`** uses the **`scales`** package in background for help with axis limits and labels. However, you can use the functions from **`scales`** directly to transform data, labels, and colors.
...
As in other chapters in this book, we'll start with some basic exploratory plots in base R. For example, we may look at the distribution of variables in each `animal_type` using the `barplot()` function. \index{graphics!barplot}
