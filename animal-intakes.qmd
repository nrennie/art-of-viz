...
This chapter introduces another new package that we haven't seen in previous chapters. We'll be using the **`lubridate`** package for manipulating dates. You'll see more examples of **`lubridate`** in @sec-nobel and @sec-r-pkgs.
...
intake_data <- longbeach |>
  mutate(
    ...
    animal_type = if_else(animal_type %in% c("dog", "cat", "bird", "wild", "reptile"), animal_type, "other")
  ) |>
  filter(
    year %in% c(2017, 2023)
  ) |>
...
max_intake <- intake_data |>
  group_by(year) |>
  summarise(total = sum(n)) |>
  pull(total) |>
  max() |>
  ceiling()
upper_limit <- round(max_intake, digits = -3)
...
n_types <- length(unique(plot_data$animal_type))
ymin_data <- plot_data |>
  ungroup() |>
...
#| echo: true
ggsave(
  filename = "longbeach-adoption.png",
  width = 5,
  height = 0.7 * 5
)
```
