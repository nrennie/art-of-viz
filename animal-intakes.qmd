...
* **`colorblindr`**: 
* **`ggforce`**:
* **`lubridate`**: a packages for manipulating and creating date variables. You'll see more examples of **`lubridate`** in @sec-nobel and @sec-r-pkgs.
* **`scales`**: You won't often have to use this package directly, but ggplot2 uses it in the background
...
heatmap(
  table(longbeach$animal_type, year(longbeach$intake_date)),
  # prevent re-ordering
  Rowv = NA,
  Colv = NA,
...
intake_data <- longbeach |>
  mutate(
    ...
    animal_type = if_else(
      animal_type %in% c("dog", "cat", "bird", "wild", "reptile"),
      animal_type,
      "other"
    )
  ) |>
...
![Initial sketches of a ideas for visualising data for a single country.](images/sketch-longbeach.png){#fig-longbeach-sketch fig-align="center" fig-alt=""}
...
use round with negative - digits to round to nearest ... . Here, we specifiically want ot round up (ceiling don't have digits)
...
Let's start with the 1980 data. We start off by creating the `0` values for the minimum in the first stacked bar using `rep(0, length(countries))` - since we need one `0` for each country in 1980. We then want to get the `ymax_2017` values from `plot_data` except the last one. We therefore use `slice_head()` to get this subset of the rows (missing the last one in each country). Remember that `plot_data` is still grouped by `iso3c`. We then stick these `ymax_2017` to the `0` we created and pass them into a new column called `ymin_2017` using `mutate()`. The code for the 2010 arc is analogous. \index{dplyr!mutate} \index{dplyr!slice\_head} \index{dplyr!pull}
...
#| fig-cap: "A 2x2 grid of charts showing simulations of what the current chart looks like with four different types of color vision deficiency."
#| fig-alt: "A 2x2 grid of charts showing simulations of what the current chart looks like with four different types of color vision deficiency, showing reasonably good contrast."
#| fig-dpi: 250
