...
For this chapter, we'll visualize data on *Medical doctors per 1,000 people* using the dataset from the Our World in Data website [@doctors_data]. You can either download the CSV file from [ourworldindata.org/grapher/physicians-per-1000-people](https://ourworldindata.org/grapher/physicians-per-1000-people) manually or you can do as we did in @sec-turbines, and download it directly into R using the URL. Simply pass the URL into the `read_csv()` function from {readr} (or `read.csv()` from base R if you prefer): \index{readr!read\_csv} \index{utils!read.csv}

```{r}
#| label: doctors-load-data-csv-show
#| eval: false
#| echo: true
doctors <- readr::read_csv(
  "https://ourworldindata.org/grapher/physicians-per-1000-people.csv"
)
```
...
The {owidR} R package [@owidR] provides an R interface to the Our World in Data website, allowing you to search and download data in R without having to navigate through the website. However, it's not much less complicated than passing in the URL to `read_csv()`. At the time of writing, the {owidR} package also hasn't been updated to use the newest API and the following code results in errors: \index{owidR!owid}
...
sysfonts::font_add_google(
  name = "Roboto Slab", family = "roboto_slab"
)
...
Let's create a custom caption that includes Font Awesome icons, as described in @sec-cats: \index{social\_caption} \index{glue!glue}
...
The color scale in @@fig-doctors-style-plot-5 is quite hard to interpret, so the choice was made to use *Fewer doctors* and *More doctors* labels instead. For this version, where the underlying data is simpler and easier to interpret, having the values on the legend would add more useful information. The title was also kept, but could probably be changed to something more informative.
