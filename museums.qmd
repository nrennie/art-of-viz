...
And we can save it to a file using `ggsave()` as we did in @sec-programming. \index{ggplot2!ggsave}

::: {#tip-museums-img-format .callout-tip}

## Choosing an image format

In @sec-programming, when saving our final plot to a file, we saved it as a PNG file. However, that's not the only option. We could also have saved in as a JPG, PDF, TIFF, or SVG (to name just a few options). So how do we decide what file type to use?
