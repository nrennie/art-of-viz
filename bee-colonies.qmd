...
* Repurpose a *dot density plot*, typically used for geographic data, to show trends over time as an alternative to bar charts; and
...
The USDA honey bee colony data was used as a TidyTuesday [@tidytuesday] dataset in January 2022 (after being suggested by [Georgios Karamanis](https://github.com/gkaramanis)), and so it can be loaded using the **`tidytuesdayR`** package: \index{tidytuesdayR!tt\_load}
...
#| fig-cap: "Simple box plot of number of bee colonies per year, showing a highly skewed distribution."
#| fig-alt: "Simple horizontal box plot of number of bee colonies per year, showing a highly skewed distribution with many large valued outliers on the right. None on the left since bounded by zero below."
#| fig-asp: 0.66
...
#| fig-cap: "Box plot of the number of bee colonies per state, showing an issue with the definition of 'state'."
#| fig-alt: "Box plot of the number of bee colonies per state, showing an issue with the definition of 'state'. California and North Dakota show higher ranges, but the highest value is for the United States i.e. is a sum of all other values."
...
It's also important to think about the ordering of the columns and rows when visualizing missing values. For example, if there is a column which holds a date, ordering the rows by date before visualizing the missing data will allow you to identify temporal patterns in the missingness. If the rows are ordered in some other way, you might miss this pattern.
...
![Initial sketch showing a grid of squares, with each square representing a combination of year and US state.](images/sketch-bees-blank.png){#fig-bees-sketch-blank fig-align="center" fig-alt="A sketch of a grid of empty squares with columns identifying years, and rows for different US states"}

Now in each grid square, we're simply trying to show the number of *things*. How might you do that? You could draw a circle, where the size of the circle shows the number of things. Or you could color each square, where the intensity of the color shows the number of things. Or you could draw lots of points, where the number of points shows the number of things as in @fig-bees-sketch. This type of chart can be called a *dot density plot*, which is commonly used for geographic data. \index{dot density plot}

We might also choose to visualize only a subset of states (either focusing on states in a specific region, or simple states we're most interested in). 

![Initial sketch showing a grid of squares, with the number of dots representing the number of bee colonies in each year and US state.](images/sketch-bees.png){#fig-bees-sketch fig-align="center" fig-alt="A sketch of a grid of squares with columns identifying years, and rows for some different US state. There are a varying number of dots in each square, and placeholder scribbles are used for title text."}
...
#| fig-alt: "Two plots side by side showing the difference between Poisson disk sampling and uniform sampling. In uniform sampling some points are close together and overlap, where as with Poisson disk they are more evenly spaced."
#| fig-subcap:
...
Returning to our `bees_grid` data, we use `unnest()` from **`tidyr`** to expand the `pts` list column into separate rows and columns, and make sure that the `state` column is a factor, ordered by the `state_levels` variable we defined earlier. \index{tidyr!unnest}  \index{base!factor} \index{dplyr!mutate}
...
This generates a dataset of the following form:
...
#| fig-alt: "A 6x6 grid with each grid square denoting a year between 2015-2020, and one of California, North Dakota, Florida, Texas, Idaho, or South Dakota. Within each square, the density of points represents the number of bee colonies. The high density for California means that the points are so closely packed that they are not distinguishable from each other."
basic_plot <- ggplot(
  data = bees_plot_data,
  mapping = aes(x = x, y = y)
) +
...
You'll notice that the points in @fig-bees-base-plot are slightly too large - the points for the California data are not even distinguishable as points; they are so dense the square simply looks like a black square! We'll reduce the size of the points in the next iteration of the plot.
...
Since we want to visualize the number of points within each grid square, there needs to be a clear distinction between the grid squares and the plot background. This means we need to make the background color of the plotting area a different color to the background color of the facet grid square. The overall plot background should be yellow but each grid square should have a black background, and then the points will be yellow again. In hindsight, perhaps `highlight_col` is not the best name to represent the `"black"` color!

Let's remake the `basic_plot` from @fig-bees-base-plot, but changing the `color` of the points and making them smaller using the `size` argument: \index{ggplot2!ggplot} \index{ggplot2!geom\_point} \index{ggplot2!aes} \index{ggplot2!facet\_grid}
...
#| fig-cap: "A 6x6 grid with each grid square denoting a year between 2015-2020, and one of California, North Dakota, Florida, Texas, Idaho, or South Dakota. Within each square, the density of points represents the number of bee colonies. A title, subtitle, and caption have been added though the subtitle runs off the side of the page."
#| fig-alt: "A 6x6 grid with each grid square denoting a year between 2015-2020, and one of California, North Dakota, Florida, Texas, Idaho, or South Dakota. Within each square, the density of points represents the number of bee colonies. A title, subtitle, and caption have been added though the subtitle runs off the side of the page."
...
We can then remove the axis text and ticks using `theme_void()`, which also removes the grid lines. Within `theme_void()`, we can also set the default typeface as our pre-defined `body_font` variable and reduce the size to ensure the facet labels fit within their allocated space. \index{ggplot2!theme\_void}
