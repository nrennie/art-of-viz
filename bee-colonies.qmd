...
* The text text colour and margin for facet labels can be modified using the `strip.text` argument. Here, the text is changed separately for the x and y facet labels because we are adding slightly different margins to the text (upper margin for x-axis facets, right margin for y-axis facets). 

* We change the title, subtitle, and caption text colour to `highlight_col` and align the caption to the left using `element_text()`. Though the subtitle is defined using `element_textbox_simple()` from {ggtext} to enable automatic line wrapping of the long text as described in @sec-programming.
