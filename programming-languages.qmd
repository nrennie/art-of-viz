...
* Create a dumbbell chart using basic geometries available in **`ggplot2`**;
* Select an appropriate font for your chart design, and load it into R for use in plots; and
* Understand how to save high quality images of plots in different formats.
...
You can alternatively read the data in via **`tidytuesdayR`** using the year and week, e.g., `languages <- tt_load(2023, week = 12)`. You can also read the data in directly from the CSV file on GitHub using `read.csv()` from base R or `read_csv()` from **`readr`**. 
...
#| fig-cap: "Basic scatter plot of how language rank varies based on last activity, showing some unusual date values."
...
Straight away this highlights the role of data visualization in exploratory analysis. This scatter plot doesn't look quite right; the `last_activity` data ranges from around -1000 to just over 2000, with most of the values close to the 2000 mark. If this is supposed to be a date column, we need to think about whether these values are correct. 
...
#| fig-alt: "The bar plot shows various categories on the x-axis, with one category having an extremely high count compared to others. The y-axis represents the frequency, with values reaching up to 3000. This suggests a highly skewed distribution with one dominant category."
...
#| fig-cap: "Basic scatter plot of how language rank varies based on last activity, showing clustering of languages with last activity before and after the year 2000."
#| fig-alt: "The scatterplot displays 'Last activity' on the x-axis, ranging from 1950 to 2010, and 'Rank' on the y-axis. The data points are densely clustered in certain time periods, with many ranks overlapping. This suggests temporal trends or concentrated activity during specific years."
par(mar = c(5.1, 4.1, 2.1, 2.1), cex.axis = 0.5, cex.names = 0.5)
...
Now, our scatter plot in @fig-languages-scatter-filter looks much more reasonable - with the first entries in the early 1950s. It also presents an interesting pattern where the programming languages are split into two different clusters. Languages with `last_activity` after 2000 are higher ranked (in the top 3000), and those with their last activity before 2000 mostly ranked below. Although this is interesting, remember that we can't really make any directional statements here. Do people prefer using languages that are more actively maintained? Or are languages more actively maintained because people prefer using them?
...
#| fig-cap: "Bar chart showing the number of different programming languages for each comment symbol, sorted in ascending in order from left to right."
#| fig-alt: "Bar chart of the number of different langauges that use different comment symbols, showing quite a skewed distribution and with double forward slashes ranked top."
#| dev: "ragg_png"
...
To visualize the time between two things happening, e.g., time between `appeared` and `last_activity_date`, we could use a dumbbell chart. Essentially, a point at each of the time points and a (usually horizontal) line connecting the two - named for their resemblance to dumbbell weights. Dumbbell charts are also commonly used to show the difference between values for two categories, e.g., heights for men and women, or blood pressure readings before and after treatment. \index{dumbbell chart}
...
A dumbbell chart for the timelines of languages that use a `#` for comments might look something like @fig-languages-sketch.
...
To show how this differs between different comment symbols, we might create small multiples of this chart - essentially creating the same plot multiple times for different subsets of the data. In **`ggplot2`**, these are called *facets*, and might look something like @#fig-languages-sketch-2.

![Sketch of a faceted dumbbell chart showing the time frame that different languages are active for a specific comment character.](images/sketch-languages-2.png){#fig-languages-sketch-2 fig-align="center" fig-alt="The hand-drawn chart titled 'Title' shows the evolution of three different languages over time. Each language is represented by a vertical line with dots and horizontal lines indicating changes and developments. The lines extend from 1950 to 2020, suggesting a significant period of linguistic evolution. The comments '#', '//', and ';' likely denote different types of changes or features within each language."}
...
#| fig-cap: "A faceted dumbbell chart showing time between language appearance and last activity, for the top 10 languages that use each of #, //, and ; as a comment symbol."
#| fig-alt: "The visualization tracks the usage of three comment tokens ('#', '//', and ';') from 1980 to 2020."
#| warning: false
...
In **`ggplot2`** (and most other plotting software) the y-axis starts with 0 at the bottom with increasing values going upward, as shown in @fig-languages-basic-plot1. However, when we visualize rankings, we usually want to put the smallest number (highest rank) at the top. So we want to reverse the y-axis here. In **`ggplot2`**, for discrete axes, we do this by passing the `rev()` (short for reverse) function to the `limits` argument of `scale_y_discrete()`. \index{base!rev} \index{ggplot2!scale\_y\_discrete}
...
#| fig-cap: "A faceted dumbbell chart showing time between language appearance and last activity, for the top 10 languages that use each of #, //, and ; as a comment symbol. High-ranked languages are at the top of the chart, and the x-axis extends from 1930 to 2030."
#| fig-alt: "The chart tracks the use of three comment tokens ('#', '//', and ';') from 1970 to 2030."
basic_plot +
...
Now @fig-languages-scales-plot looks a little bit like the sketch in @fig-languages-sketch-2. But it doesn't look great yet. It needs a bit more work to improve both the aesthetics and the clarity of the visualization. 
...
We can define some variables to store our colors in: a variable called `bg_col` that's dark gray we'll use for the background; and a variable called `main_col` that's a hex code for a bright green we'll use for the text and other geometric elements.

::: {#tip-languages-colors .callout-tip}

## Colors in R

The dark gray, `"gray5"`, is a pre-defined color in R. Run `colors()` to see the list of all `r length(colors())` pre-defined colors available. \index{color}
...
The point character (`pch`) for `geom_point()` is also set to `22` - a square which allows you to color the square and its outline different colors. The outline `color` is set to our green `main_col` and the square `fill` color is set to the dark gray `bg_col` in @fig-languages-basic-plot2.

```{r}
#| label: fig-languages-basic-plot2
#| fig-cap: "A faceted dumbbell chart showing time between language appearance and last activity, for the top 10 languages that use each of #, //, and ; as a comment symbol with green lines and green-outlined dark gray points."
#| fig-alt: "The chart tracks the use of three comment tokens ('#', '//', and ';') from 1970 to 2030. The lines are green, showing a clear distinction between the time periods."
...
Since our visualization doesn't give information on how many languages use each comment symbol and just focuses on the top 10, we might want to present this information in the subtitle instead:
...
Let's add the title, subtitle, and caption to the plot using the `labs()` function: \index{ggplot2!labs}

```{r}
#| label: fig-languages-text_plot
#| fig-cap: "A faceted dumbbell chart showing time between language appearance and last activity, for the top 10 languages that use each of #, //, and ; as a comment symbol. An unformatted title, subtitle, and caption have been added."
#| fig-alt: "The chart titled 'Programming Languages' tracks the use of three comment tokens (#, //, and ;) from 1970 to 2030. The lines are green, showing a clear distinction between the time periods."
...
We can also set the `base_family` equal to `body_font` - the variable we defined earlier to store the name of the typeface we want to use for the text. \index{ggplot2!theme\_minimal}

```{r}
#| label: fig-languages-minimal-theme
#| fig-cap: "A faceted dumbbell chart showing time between language appearance and last activity, for the top 10 languages that use each of #, //, and ; as a comment symbol. The default font has been changed, and the font made slightly smaller."
#| fig-alt: "The chart titled 'Programming Languages' tracks the use of three comment tokens (#, //, and ;;) in 4,303 programming languages from 1970 to 2030. The lines are green, showing a clear distinction between the time periods."
...
We can make some further adjustments to the styling of @fig-languages-minimal-theme by editing individual arguments in the `theme()` function. When editing `theme()` arguments, many arguments are specified as a type of element: `element_text()`, `element_line()`, `element_rect()`, `element_grob()`, `element_render()`, or `element_blank()`. 

#### Background color

We can edit the background color using the `panel.background` and `plot.background` arguments. The `panel.background` refers to the area within the plot, e.g., the area that is gray by default. The `plot.background` refers to the area outside of this. Both of these are specified using `element_rect()` since they are effectively just rectangles. We want to change both the `fill` (inner color of the rectangle) and `color` (outline color of the rectangle to our chosen background color.)
...
#| fig-cap: "A faceted dumbbell chart showing time between language appearance and last activity, for the top 10 languages that use each of #, //, and ; as a comment symbol, with a dark gray background and green text."
#| fig-alt: "The chart titled 'Programming Languages' tracks the use of three comment tokens (#, //, and ;;) in 4,303 programming languages from 1970 to 2030. The lines are green, showing a clear distinction between the time periods. The background is black with green text."
theme_plot1 +
  theme(
    # Background color
...
We also need to deal with the fact that the subtitle text is too long and is running off the side of the page in @fig-languages-style1. There are a few options for this:
...
* The **`marquee`** package provides a similar functionality to **`ggtext`**, which we'll cover in @sec-turbines.
...
#| fig-cap: "A faceted dumbbell chart showing time between language appearance and last activity, for the top 10 languages that use each of #, //, and ; as a comment symbol. The subtitle text now wraps to the width of the page."
#| fig-alt: "The chart titled 'Programming Languages' tracks the use of three comment tokens (#, //, and ;;) in 4,303 programming languages from 1970 to 2030. The lines are green, showing a clear distinction between the time periods."
...
Finally, since we've removed the y-axis labels in @fig-languages-style2, we need to add in some labels using `geom_text()`. The `geom_text()` function has three required aesthetics: the `x` and `y` coordinates of where the label should be placed, and `label` which defines what text is written. Here, we use the language `title` as the label, and put a label in each rank category, `n`, on the y-axis. We override the `x` aesthetic to place all labels in the same horizontal alignment - the exact value will take a little bit of trial and error to get right. 
...
#| fig-cap: "A faceted dumbbell chart showing time between language appearance and last activity, for the top 10 languages that use each of #, //, and ; as a comment symbol, with labels added to denote the language."
#| fig-alt: "The chart titled 'Programming Languages' tracks the use of three comment tokens (#, //, and ;;) in 4,303 programming languages from 1970 to 2030. The lines are green, showing a clear distinction between the time periods. Labels details the language name."
theme_plot2 +
  geom_text(
...
#| fig-cap: "A faceted dumbbell chart showing time between language appearance and last activity, for the top 10 languages that use each of #, //, and ; as a comment symbol, with labels added to denote the langauge and rank."
#| fig-alt: "The chart titled 'Programming Languages' tracks the use of three comment tokens (#, //, and ;;) in 4,303 programming languages from 1970 to 2030. The lines are green, showing a clear distinction between the time periods. Labels detail the language name and overall rank."
theme_plot2 +
  geom_text(
    data = plot_data,
...
And we're finished with @fig-languages-add-better-labels!
...
![](images/languages.gif){fig-align="center" fig-alt="Gif of the process of building the chart"}
