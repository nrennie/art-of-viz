...
```{r}
#| label: langauges-plot-data-head
head(plot_data)
```
...
Let's start by loading {ggplot2} using the `library()` command.

One of the ... things about {ggplot2} is that .... . And so we'll be iteratively adding to our plots. saving as objects 
...
The dark grey, `"gray5"`, is a pre-defined color in R. Run `colors()` to see the list of all `r length(colors())` pre-defined colors available.
...
#| fig-cap: "A facetted dumbbell chart showing time between language appearance and last activity, for the top 10 languages that use each of #, //, and ; as a comment symbol with green lines and green-outlined dark grey points."
...
Let's add these new and improved labels to our plot using `geom_text()`: \index{ggplot2!geom\_text}
...
You can set the width and height in `ggsave()`: \index{ggplot2!ggsave}
...
By default, the width and heigh is given in inches (though you can change this with the `unit` argument) and the resolution (set with the `dpi` argument) is 300. If you've saved your final plot to a variable in R, you can pass this directly to `ggsave()` instead of printing the object first:

```{r}
#| label: langauges-save-2
#| eval: false
#| echo: true
