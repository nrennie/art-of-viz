...
You can alternatively read the data in via {tidytuesdayR} using the year and week. For example `languages <- tidytuesdayR::tt_load(2023, week = 12)`. You can also read the data in directly from the CSV file on GitHub using `read.csv()` or `read_csv()` from {readr}. 
