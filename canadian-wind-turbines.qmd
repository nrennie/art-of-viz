...
* Instead of plotting squares, we could use icons. For example, we could plot an icon of a wind turbine. This variation of a waffle chart can be referred to as a *pictogram*.

\index{pictogram} 

## Preparing a plot

In order to make a draft of @sketch-turbines.png, there are a few things we need to prepare:
...
Before we jump into plotting, we need to do one more thing. In @fig-sketch-turbines, we decided we would use icons in the waffle chart instead of just colored grid squares. So we need to fid a way of using icons in R. 
...
new_data <- base_build$data[[1]] |>
  dplyr::rename(Region = PANEL) |> 
  dplyr::mutate(
    Region = factor(Region, labels = levels(plot_data$Year_Group))
  )
```
