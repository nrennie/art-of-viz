...
This is quite a significant difference! Let's find a way to communicate this and highlight this disparity. 

### Exploratory sketches

When thinking about how to construct a visualisation, it's important to consider multiple factors [@signif2024a]:

* What message are you trying to convey?
* Who are you trying to reach and what is their background?
* What is the purpose of the visualisation?
* Does the way you've built that visualisation support the purpose?

Let's say that the purpose of the visualisation is to highlight the gender disparity in Nobel Prize awardees. Yes, the bar chart of how many male and female Nobel Laureates there are shows the disparity very clearly. It's a simple message. But it's not eye-catching. It's not a visualisation that really makes a reader stop and pay attention. And it aggregates data down to just two categories - ignoring that these are individual people. 
...
A bar chart would be a quicker, more efficient way of displaying how many male and female Nobel Laureates there are. A visualisation of the names of every single Nobel Laureate might be a quicker, more efficient way of catching someone's attention. This alternative visualisation doesn't sacrifice accuracy of information, but it might take a reader a little bit longer to digest the information. Whether that's a sacrifice worth making comes back to what the purpose of the visualisation is.

## Preparing a plot

So let's start making our custom visualisation! We start by defining a variable for the award category we're interested in - Physics. This first step might seem a little odd since we've only queried the API for physics data anyway, but when you get to the last section of this chapter, it'll make more sense!
...
When we're converting our code into a function, we need to think about what a user has to tell us, in order for the code to work. In the sales histogram example above, a user would need to specify which month they want the plot for. In our example of the Nobel Prize visualisations, a user need to specify: 
