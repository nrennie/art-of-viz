...
You can see that the output of the `social_caption()` is a pretty incomprehensible string that would've been quite tricky to type out manually - are't we glad we built a function to do it for us! We can then use our `source_caption()` function from @sec-turbines, and pass the output from the `social_caption()` function into the `graphic` argument. We also place them on separate lines by using `<br>`. \index{source\_caption}
