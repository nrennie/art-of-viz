...
* Create a *bubble plot* using core **`ggplot2`** functions; 
* Design data-driven but aesthetically pleasing annotations to highlight interesting data points; and
* Add social media icons to captions as a concise way of giving attribution for the graphic.
...
All of the packages in this chapter are ones that we've used in other chapters. However, we'll be going into a bit more depth on some and using new functions from others!
...
The data was used as a TidyTuesday dataset in January 2023 (after being suggested by [Tom Mock](https://github.com/jthomasmock)), and so it can be loaded with the `tt_load()` function from **`tidytuesdayR`** [@tidytuesdayR]. There are two datasets included, which we'll read in as `cats` and `cats_reference`. \index{tidytuesdayR!tt\_load}
...
With the `cats` data, we could focus on the spatial patterns and look at the range of where cats travel. For example, how far does the average cat travel in a day? Alternatively, we could focus on the `cats_reference` data and look at relationships between different behaviors and characteristics of cats. For example, do older cats spend more time indoors? Or do cats who live with other cats bring home more prey?
...
Let's start by digging into the geographic `cats` data. We can make a very quick plot of the coordinate data using `plot()`. \index{graphics!plot}
...
#| fig-alt: "Scatter plot showing the geographic coordinates obtained from the GPS data attached to pet cats. The lack of background map makes it hard to interpret without context, but some clusters can be seen."
...
#| fig-alt: "Histogram of the ground speeds recorded from GPS sensors, showing a highly skewed distribution with some very high speeds! The x-axis goes up to 250000 metres per second."
...
#| fig-alt: "Bar chart showing the number of cats that fall into each of the five categories for the average amount of time they spend indoors. Although the data may at first appear numeric, it is in fact categorical data. The distribution follows a bell curve."
...
#| fig-alt: "Scatter plot of the relationship between the age of pet cats and the average amount of time they spend indoors. There seems to be fewer data points than cats shown on the plot."
par(mar = c(5.1, 4.1, 2.1, 2.1))
plot(
...
Let's take the idea from @fig-cats-scatter but develop into a much more effective visualization, making particular use of text and annotations. We'll update the chart to make a bubble chart. But we'll also add annotations to highlight interesting aspects of the data e.g. the oldest cat, or the average amount of time spent indoors. @fig-cats-sketch gives a basic illustration of how we might develop it. \index{bubble chart}

![Initial sketch of a bubble plot with annotations, to show the relationship between the age of pet cats and how much time they spend indoors.](images/sketch-cats.png){#fig-cats-sketch fig-align="center" fig-alt="Initial sketch of a bubble plot with annotations, to show the relationship between the age of pet cats and how much time they spend indoors. Boxes and arrows demonstrate captions highlighting specific points."}
...
The `hrs_indoors` column contains five categories for time spent indoors: `2.5`, `7.5`, `12.5`, `17.5`, and `22.5`. Since these category names are numbers, they are encoded in R as numeric columns instead of categories, so we start by converting the `hrs_indoors` column to a `factor()`. To know how big the *bubbles* of our plot will be, we need to know how many cats there are for each combination of age and indoor time. We `group_by()` the two relevant columns and then use `count()` from **`dplyr`** to count the number of each category combination, before ungrouping with `ungroup()`. \index{dplyr!select} \index{dplyr!mutate} \index{base!factor} \index{dplyr!group\_by} \index{dplyr!count} \index{dplyr!ungroup} 

There's one cat (`r cats_reference$animal_id[is.na(cats_reference$age_years)]`) whose age is unknown, so we use `drop_na()` from **`tidyr`** to remove this value. See @sec-bees and @sec-r-pkgs for other discussions on visualizing and dealing with missing values in your data. \index{tidyr!drop\_na} 
...
#| fig-alt: "Bubble plot of the relationship between the age of pet cats and the average amount of time they spend indoors, with the size of circles indicating the number of cats in each category. Legend values are 2.5, 5, and 7.5."
basic_plot <- ggplot(
  data = plot_data,
  mapping = aes(
    x = age_years,
...
Since we aren't mapping any columns in the data to `color` or `fill`, we don't need to specify a color palette. Instead, we'll simply choose a text color (dark gray), background color (white), and highlight color (purple). \index{color}
...
#| fig-alt: "Bubble plot of the relationship between the age of pet cats and the average amount of time they spend indoors, with the size of purple circles indicating the number of cats in each category. The legend for size is now presented using whole numbers of 3, 6, and 9."
...
Here, we’ll use a *fun* typeface for the title, but keep using a clean and minimal typeface for the rest of the text. For the `body_font`, we'll use *Ubuntu* as we did in @sec-longbeach. For the `title_font`, we'll use *Chewy*, an almost cartoonish typeface that vaguely resembles handwriting. Both are Google Fonts and so we load them using `font_add_google()` as we've done in previous chapters, and set up the relevant **`showtext`** options. \index{sysfonts!font\_add\_google} \index{showtext!showtext\_auto} \index{showtext!showtext\_opts} \index{fonts}
...
#| fig-alt: "Bubble plot of the relationship between the age of pet cats and the average amount of time they spend indoors. A textbox in the top right indicates the oldest cat called 'Boots'."
...
#| fig-alt: "Bubble plot of the relationship between the age of pet cats and the average amount of time they spend indoors. A textbox in the top right indicates the oldest cat called 'Boots', with an arrow pointing towards the data point for this cat."
...
## Font Awesome icons in R

The **`fontawesome`** package [@fontawesomeR] makes it easier to use Font Awesome icons in R Markdown, Quarto, and Shiny outputs created with R. The package allows you to insert icons as SVG or PNG images, and provides functions for writing HTML code that inserts icons using `<i></i>` tags in Shiny applications.
...
The *`ggbranding`* package [@ggbranding] allows you to add a some icons for attribution to your plots without constructing the strings yourself.
...
#| fig-cap: "Bubble plot of the relationship between the age of pet cats and the average amount of time they spend indoors. A title, subtitle, and caption have been added, although the caption appears with raw HTML code, rather than correctly formatted."
#| fig-alt: "Bubble plot of the relationship between the age of pet cats and the average amount of time they spend indoors. A title, subtitle, and caption have been added, although the caption appears with raw HTML code, rather than correctly formatted."
...
#| fig-alt: "Bubble plot of the relationship between the age of pet cats and the average amount of time they spend indoors. The text is now all correctly formatted, and the legend appears inset with a clear border between it and the plot."
text_plot +
  theme_minimal(
...
@fig-cats-styled-plot is a very clean looking chart and, though annotations have been added, it doesn't look too busy or complex. We could (carefully) add further annotations to perhaps highlight the youngest cats or the cats who spent the least amount of time indoors. 
...
![](images/cats.gif){fig-align="center" fig-alt="Gif of the process of building the chart"}
