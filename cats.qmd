...
The glue_icon function creates a styled HTML snippet for a social media icon and its associated text, using the glue package for string interpolation. It combines two spans: one for the icon (styled with the "Font Awesome 6 Brands" font and a customizable icon_color) and another for the social media text (styled with customizable font_family and font_color), separated by spaces (&nbsp;) and followed by extra spacing (&emsp;).
