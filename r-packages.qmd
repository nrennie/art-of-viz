...
Then pass into `spot_funs_files()` from {funspotr} [@funspotr]
...
The `r_pkgs` data has `r nrow(r_pkgs)` rows and `r ncol(r_pkgs)` columns.
...
The obvious question here is: which packages are used most often? Let's start by counting the number of package uses with the `table()` function, and using `sort()` to arrange packages from least to most used. \index{base!table} \index{base!sort}
...
#| fig-cap: "Ordere bar chart showing the number of times each package was used in 2023. The 'ggplot2' package is used most."
#| fig-asp: 1
barplot(
...
This is very similar to the initial plots created in @sec-museums.

Although `"tibble"` is a level in our `pkgs` factor column, no observations of that factor level exist. To make sure it still appears as an (empty) facet, we set `drop = FALSE`. Otherwise, only 7 faceted plots would be shown.
...
You can do a very similar thing using the {marquee} package [@marquee] instead of {ggtext}. 
