...
We then pass this data frame into `spot_funs_files()` from {funspotr} to get the list of function and packages uses in each file listed in `files_to_check`. Since we want to look at total package use, we set `show_each_use = TRUE` to make sure that individual rows are returned each time a function is used (rather than just once for an entire file). \index{funspotr!spot\_funs\_files}
...
The `r_pkgs` data has `r nrow(r_pkgs)` rows and `r ncol(r_pkgs)` columns. The `funs` column contains names of each function used, `pkgs` has the name of the package that function comes from. The remaining two columns are the `relative_paths` and `absolute_paths` columns that identify which file the function was found in. Let's have a quick look at the data using `head()`: \index{utils!head}
...
The obvious questions to start with here are: which packages are used most often? Which functions are used most often?
...
One thing that you may have noticed is that, for some reason, in the data, the package is given as `"ggplot"` rather than `"ggplot2"`. So let's fix that using `mutate()` and the `if_else()` function from {dplyr}. We can overwrite the existing `pkgs` column, where (i) if the value is currently `"ggplot"`, we replace it with `"ggplot2"`; (ii) otherwise, we leave the existing value in the `pkgs` column. \index{dplyr!mutate} \index{dplyr!if\_else}
...
::: {#tip-packages-ifelse .callout-tip}
...
* The number of package uses per day for each core {tidyverse} packages; and 
...
Now that we have a first draft of the plot, it's time to work on polishing it. We'll make some adjustments to the fonts, colors, text, scales, and themes - before we move on to editing the facets labels to create a visualisation like @fig-packages-sketch-2.
...
We then edit the `plot.background` and `panel.background` to have the background color we defined earlier (as `bg_col`) by using the `fill` and `color` arguments in `element_rect()`.

bit more here on facet styling

 \index{ggplot2!element\_rect}   \index{ggplot2!element\_blank} \index{ggplot2!element\_line} \index{ggtext!element\_textbox\_simple}
...
bit here on text 
axis styling
 \index{ggplot2!theme} \index{ggplot2!element\_rect} \index{ggplot2!element\_text} \index{grid!unit} \index{ggplot2!element\_blank} \index{ggplot2!element\_line} \index{ggtext!element\_textbox\_simple}
