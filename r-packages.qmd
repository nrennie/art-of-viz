...
Then, we use `mutate()` to convert the `pkgs` column of our data from a character string to a factor (ordered category) and set the `levels` equal to our new `pkgs_levels` vector: \index{dplyr!mutate} \index{base!factor}
