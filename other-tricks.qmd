...
#| echo: false
#| warning: false
base_plot <- readRDS("data/base_plot.rds")
```
...
These two things aren't currently compatible. We've already built the plot with `geom_pictogram()` so the x- and y- coordinates have already been calculated. Luckily, we can use the `ggplot_build()` function to extract from the `base_plot` we created in @sec-turbines. \index{ggplot2!ggplot\_build}
...
If we wanted to color the icons based on the `Region` variable, we would need to have multiple copies of the image in different colors. We could combine the information in @sec-seafood about manipulating and re-coloring images, along with the information in @sec-r-pkgs about defining image paths based on data values to do this programmatically.
