...
#| eval: false
c("camcorder", "dplyr", "ggpattern", "ggplot2")
```
...
We've already built the plot with `geom_pictogram()` so the x- and y- coordinates have already been calculated. Luckily, we can use the `ggplot_build()` function to extract from the `basic_plot` we created in @sec-turbines. \index{ggplot2!ggplot\_build}
