...
As you've already seen in @sec-chap4, creating functions for code you reuse frequently can save time and space. In @sec-chap4, we created the function `social_caption()` for adding a plot caption that contained Font Awesome icons. This function has then been used in every subsequent chapter (and therefore also the template file described in the previous section). What other helper functions might be useful? \index{social\_caption} 
