...
This package introduces only one new package, **`geofacet`**, which is used for arranging a grid of plots in the shape of a geographic area. The package comes with many pre-defined grids for different countries and regions, but you can also create your own custom grids. \index{geofacet}

## Data

The MIT Election Data and Science Lab collects, analyzes, and shares data and results about elections in the United States of America, with the aim to improve American democracy. This includes data on elections to the U.S. House of Representatives, at the constituency level [@elections_data]. The data is regularly updated, and a version of this dataset containing information about elections from 1976 to 2022 was used as a [TidyTuesday dataset in November 2023](https://github.com/rfordatascience/tidytuesday/tree/main/data/2023/2023-11-07). We can again load the data in using the `tt_load()` function from **`tidytuesdayR`**: \index{TidyTuesday} \index{tidytuesdayR!tt\_load}
...
There are `r format(nrow(house), big.mark = ",")` rows and `r ncol(house)` columns in the data. Let's take a look at the first few rows of the data using `head()` (or `View()` if you prefer): \index{utils!head}
...
There doesn't seem to be an especially interesting pattern in votes over time in @fig-elections-scatterplot, at least at first glance. However, it has highlighted a potential data issue. In most years, the highest values are around 300,000 but there are two observations in 2022 where the values are well over 1,000,000. We had a similar issue in @sec-bees, where some observations were the sum of other observations. This doesn't seem to be the case here. For now, let's exclude these and look at data where the number of candidate votes is below 600,000. We might also be interested in how the number of candidate votes varies by party, and we can use the `boxplot()` function to explore whether or not there are big differences. \index{graphics!boxplot}

```{r, echo=-1}
#| label: fig-elections-boxplot
#| fig-cap: "Box plot showing the number of candidate votes for each party, with many party labels being hidden due to the large number of unique parties (with small numbers of votes). Some of the longer names are cropped out, but several misspellings are still visible."
#| fig-alt: "Box plot showing the number of candidate votes for each party, with many party labels being hidden due to the large number of unique parties (with small numbers of votes). Some of the longer names are cropped out, but several misspellings are still visible."
#| fig-asp: 1.2
...
In @fig-elections-boxplot there are a couple of interesting points to note. First, there are many, many unique `party` names, because every independent candidate is listed as a different party name (including some which are misspelled). This means that not all party names can actually by read off from the chart. Second, there are a lot of outliers shown in the data, which is highly skewed. Finally, there are a couple of parties that have a much higher numbers of votes. Inspecting the data confirms that these are, as we might guess, the Democratic Party and the Republican Party.
...
We're once again going to create a faceted plot, as we did in @sec-programming, @sec-museums, and @sec-longbeach. There are two key differences in this plot:
...
We now have a basic plot for one of the facet values in @fig-elections-single-plot.

::: {#tip-elections-piping .callout-tip}

## Piping data into `ggplot()`

Since the first argument of `ggplot` is `data`, this means that we *could* pipe the data into the `ggplot()` function rather than explicitly stating it as the first argument. For example, the code above could be rewritten as:
...
Though this may arguably look neater than nesting `filter()` inside `ggplot()`, you're unlikely to see any other examples of piping into `ggplot()` in this book for two reasons:
...
#| fig-cap: "Faceted area chart arranged in a rectangular grid shape, showing percentage of votes per party over time. The area charts cannot be seen due to the size of the facet labels."
#| fig-alt: "Faceted area chart arranged in a rectangular grid shape, showing percentage of votes per party over time. The area charts cannot be seen due to the size of the facet labels."
#| warning: false
...
* In order to map the data to the correct facet position on the grid, one of the columns in the data must align with the value in the `code` column of your chosen grid. In the `"us_state_grid2"` grid, the `code` column contains the two-letter state abbreviations which match the `state_po` column in our data. If you didn't have the two-letter abbreviations, you may need to do some data joining to attach them.

If you have additional *states* (e.g., Puerto Rico can sometimes be listed in a US state column, even though it isn't one) or are missing some states, you can still use `facet_geo()`. However, it will only plot states listed in the facet grid, and you may get a warning message.
...
We still have the same problem of not being able to see the details of the chart in @fig-elections-geofacet-plot, but you can see from just the facet titles that the plots are now arranged in a grid that resembles the USA. 
...
We write text for the subtitle, using `glue()` to add HTML styling for colored text as we did in @sec-cats and @sec-lemurs. The colored text serves as a legend (though we'll later also add our own custom legend), and the subtitle text provides information about what the chart shows. The colored text is formatted as bold face text to make it a little bit easier to read. As in @sec-time-zones, we'll add the information about the source of the data on a new line at the end of subtitle, rather than in the caption. We also add the title text directly in the subtitle variable to enable easier placement of the text, again as we did in @sec-time-zones. \index{glue!glue}
...
As in the main plot, we also set `expand = FALSE` inside `coord_cartesian()` to remove the additional space around the edges. We use `theme_void()` to remove any additional chart elements, since the annotations convey all the necessary information. \index{ggplot2!coord\_cartesian} \index{ggplot2!theme\_void}

```{r}
#| label: fig-elections-inset-plot
#| fig-cap: "Custom legend plot consisting of a zoomed in version of the California facet with additional annotations indicating the time period and categories represented by different colors."
#| fig-alt: "Custom legend plot consisting of a zoomed in version of the California facet with additional annotations indicating the time period and categories represented by different colors."
#| fig-asp: 1
#| fig-width: 3
#| fig-align: center
p_inset <- ca_plot +
...
Our custom legend in @fig-elections-inset-plot is now ready to add to the main plot.
...
We can then save @fig-combine-plots to a file with `ggsave()`! \index{ggplot2!ggsave}
...
ggsave(
  ...
  plot = final_plot,
  width = 5, height = 0.5 * 5
)
```

## Reflection

This visualization summarizes a large amount of complex information in a way that is fairly intuitive - the colors match recognizable party colors, the grid is arranged in a shape that is instantly identifiable, and the area charts clearly indicate vote share over time. However, there are a few more improvements that could be made, and other elements we may want to consider. 

Though the use of state two-letter abbreviations saves space and avoids excessive text, it is more difficult for a reader to easily look up the state they are interested in - especially if they are less familiar with US geography. Perhaps adding some smaller text under the abbreviations, or creating an interactive version that provides state names upon hovering, would make it easier. 
...
The colors are not the most accessible since they are of similar intensity. If we were to use the `cvd_grid()` function from **`colorblindr`** as we did in @sec-longbeach, these colors might look much less distinguishable. Choosing a combination of a lighter blue and bright red, or lighter red and bright blue would retain the intuitiveness of the colors but improve accessibility. \index{color} \index{accessibility} \index{colorblindr!cvd\_grid}
